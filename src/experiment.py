...
class HiddenLoopExperiment:
    ...
    def hidden_loop_experiment(self, adherence=0.2, usage=0.1, step=10):
        ...
        self.X_tr, self.X_tst, self.y_tr, self.y_tst = model_selection.train_test_split(self.X_curr,
                                                                                        self.y_curr)
    ...
    def make_hist(self, step):
        ...
        # dens_func_train = plt.hist(data_train, density=True, label='Training')
        # dens_func_test = plt.hist(data_test, density=True, label='Test')
        pred_tr = self.gbr.predict(self.X_tr)
        pred_tst = self.gbr.predict(self.X_tst)
